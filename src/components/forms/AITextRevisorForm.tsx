...
import { Box, Button, Chip, FormControl, Grid, InputLabel, MenuItem, OutlinedInput, Select, type SelectChangeEvent, Slider, TextField, Typography } from '@mui/material'
...
export default function AITextRevisorForm (props: {
  ...
  const handleRevisionTypeChange = (event: SelectChangeEvent<typeof revisionTypes>): void => {
    const {
      target: { value }
    } = event
    setRevisionTypes(
  ...
  const handleSubmit = (event: any): void => {
    ...
    if (revisionTypes.length > 0) {
      requestBody.revisionType = revisionTypes
    }
    console.log(`Sending request ${JSON.stringify(requestBody)} to ${constants.API_URL}${constants.OPEN_AI_TEXT_REVISOR_API_PREFIX}`)
    axios.post(`${constants.API_URL}${constants.OPEN_AI_TEXT_REVISOR_API_PREFIX}`, requestBody)
      .then((response) => {
        console.log(response)
        let revisions = ''
        for (const [i, revision] of response.data.revisedTextList.entries()) {
          revisions += `Revision ${(i as number) + 1}:\n${revision as string}\n\n`
        }
